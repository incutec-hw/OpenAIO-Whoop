...
## Parts imported (LCSC, project-local `whoop` library)

Current locations after the 2026-06-12 restructure: symbols in `hardware/whoop.kicad_sym`, footprints in `hardware/whoop.pretty/`, 3D models in `hardware/whoop.3dshapes/`, all under the `whoop` lib-table nickname.

| Symbol | Footprint | 3D | LCSC | Role |
|---|---|---|---|---|
...
- 3D-model paths are `${KIPRJMOD}`-relative.
...
## Market bar for a 2026 whoop AIO (from competitive research, see MARKET-RESEARCH-2026-06.md)
...
**Decision: the Bluejay ESC has no gate-driver IC, EFM8 GPIO direct drive only, no board space.** This locks the power stage to complementary P+N per phase (high-side N is impossible without a driver/bootstrap). Consequences:
...
### Revision 4: BetaFPV teardown observations close the architecture question
...
**RX MCU density option: ESP8685H4 (C4944062, QFN-28 4×4, 872 JLC stock)**, ESP32-C3 die with in-package 4 MB flash in a 4×4 (vs 5×5 QFN-32 ESP32-C3FH4); the ESP8285 analog for the C3 generation. Runs the same ELRS ESP32-C3 targets; OpenRX already had its datasheet on file. New footprint + fewer GPIO, RX section relayout. Imported into `whoop` lib.
...
All three imported into `whoop` lib (SIA527DJ-T1-GE3, AON2406, YJQ1216A); SiA527DJ datasheet local. **Open choice: SiA527DJ dual (12 pkgs, simpler) vs AON2406+YJQ1216A pair (24 pkgs, ~half the conduction loss).** DMC1229UFDB-7 below remains the fallback dual.
